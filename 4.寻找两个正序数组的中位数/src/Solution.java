    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        for (int i = 0, j = 0 ; ;){
            ...
            if (nums1[i] < nums2[j])
                ...
                nums3[i + j] = nums2[j++];
            //System.out.println(nums3[i+j-1]);
            }
        }

        //取中位数
        if (0 == nums3.length % 2)
            midNum = (nums3[nums3.length / 2] + nums3[nums3.length / 2 - 1]) / 2.0;
        else midNum = nums3[nums3.length / 2];
        return midNum;
    }
}
...
所以必定至少会有一个数组是存在第K/2个数字的。最后就是二分法的核心啦，
